...
| `tests/move-actions.spec.ts` | @critical — Accept/Complete/Save/Not relevant persist across reloads into the right tabs; no Send affordance; API rejects `sent`; draft edits |
| `tests/onboarding-voice.spec.ts` | Feature detection, mic denial fallback, transcript append + voice input mode |
...
| `tests/helpers/` | `env.ts` (.env.local parsing), `db.ts` (service-role seeding/assertions, refuses non-local Supabase), `auth.ts` (UI login/signup), `speech.ts` (deterministic SpeechRecognition fake) |
...
- AI failures are injected with the `x-koda-test-ai: fail` header (honored only in mock mode outside production; it can only cause failures).
